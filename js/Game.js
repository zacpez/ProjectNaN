...
var Game = function() {
    ...
    this.initialize = function(canvasElementId, callback) {
        ...
        this.gameCam.setSourceTile(
            Math.floor(this.gameCam.getPos()
                .x / TILE_SIZE) - EDGE_BUFFER,
            Math.floor(this.gameCam.getPos()
                .y / TILE_SIZE) - EDGE_BUFFER
        );
            ...
            .mousemove(function(event) {
                ...
                if (pn.game.dragging) {
                    ...
                    pn.game.gameCam.setSourcePixel(
                        -(TILE_SIZE - (pn.game.gameCam.getPos()
                            .x % TILE_SIZE)),
                        -(TILE_SIZE - (pn.game.gameCam.getPos()
                            .y % TILE_SIZE))
                    );
                    ...
                    pn.game.gameCam.setSourceTile(
                        Math.floor(pn.game.gameCam.getPos()
                            .x / TILE_SIZE) - EDGE_BUFFER,
                        Math.floor(pn.game.gameCam.getPos()
                            .y / TILE_SIZE) - EDGE_BUFFER
                    );
    ...
    this.loop = function() {
        ...
        window.requestAnimationFrame(pn.game.loop);
    }
    ...
    this.start = function() {
        pn.socket.addUser('add_player', {
            player: JSON.stringify(pn.game.player)
        });

        for (var i = 0; i < GENERATION.WIDTH; i += 1) {
            for (var j = 0; j < GENERATION.HEIGHT; j += 1) {
                var x = (-(TILE_SIZE + (pn.game.gameCam.getPos()
                    .x % TILE_SIZE))) + ((i - pn.game.gameCam.getSourceTile()
                    .x) * TILE_SIZE);
                var y = (-(TILE_SIZE + (pn.game.gameCam.getPos()
                    .y % TILE_SIZE))) + ((j - pn.game.gameCam.getSourceTile()
                    .y) * TILE_SIZE);
